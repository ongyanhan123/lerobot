...
class RealSenseCamera(Camera):
    ...
    def _find_serial_number_from_name(self, name: str) -> str:
        ...
        if len(found_devices) > 1:
            serial_numbers = [
                str(dev.get("id", dev.get("serial_number", "<unknown>")))
        ...
        serial_number = str(found_devices[0].get("id", found_devices[0].get("serial_number")))
        if not serial_number:
            raise RuntimeError(f"Camera info for '{name}' has no serial number/id.")
